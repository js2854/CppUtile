...
bool RegexMatch(const string &i_strRegx, const string &i_strSrc, SInt32 i_s32Flags, StringVector &o_vecMatchs)
{
    ...
    regmatch_t tMatchs[MAX_REGEX_MATCH_NUM+1] = {{0}};
    err = regexec(&reg, i_strSrc.c_str(), MAX_REGEX_MATCH_NUM, tMatchs, 0);
    if (REG_NOMATCH == err)
    {
    ...
    // 跳过完整匹配项，即tMatchs[0]
    for (UInt32 i = 1; i < MAX_REGEX_MATCH_NUM && tMatchs[i].rm_so != -1; i++)
    {
        UInt32 u32Len = tMatchs[i].rm_eo - tMatchs[i].rm_so;
        if (u32Len > 0)
        {
            const string &strMatch = i_strSrc.substr(tMatchs[i].rm_so, u32Len);
